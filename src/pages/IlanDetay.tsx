...
const IlanDetay: React.FC<IlanDetayProps> = ({ id }) => {
  ...
  const [images, setImages] = useState<{ url: string; idx: number }[]>([]);
      ...
      try {
        ...
        if (data) {
          ...
          if (data.listing_images) {
            const sortedImgs = [...data.listing_images].sort((a, b) => (a.idx || 0) - (b.idx || 0));
  ...
  // Teknik özelliklerin listesi (Değerler String yapılarak TypeScript hatası çözüldü)
  const specList = listing?.specs ? Object.entries(listing.specs)
    .filter(([key, value]) => value && value !== "" && key !== "priceLine")
    .map(([key, value]) => ({ 
  ...
  if (loading) return <div className="py-32 text-center text-[#8B92A4] font-light tracking-widest uppercase italic">İlan Detayları Hazırlanıyor...</div>;
  if (!listing) return <div className="py-32 text-center text-[#8B92A4]">İlan bulunamadı.</div>;
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 py-12">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
          <div className="flex flex-col gap-2">
            <a href="#/portfoyler" className="flex items-center gap-2 text-[11px] font-semibold tracking-widest uppercase text-[#8B92A4] hover:text-[#112769] transition-colors mb-4 group">
          ...
          <div className="text-[2rem] font-light text-[#112769] tracking-tighter">
            {formatTRY(listing.price_tl)}
          </div>
        </div>
    ...
    >
      {images.length > 0 ? images.map((img, i) => (
        ...
        <div className="w-full h-full shrink-0">
          <img src={listing.cover_url || ""} className="w-full h-full object-cover" alt="Kapak Görseli" />
        </div>
      )}
    </div>
      ...
      {images.map((img, i) => (
        <button 
          ...
          className={`relative shrink-0 w-24 md:w-32 aspect-[4/3] transition-all duration-500 border-2 ${
            activeImage === i 
            ? "border-[#C5A572] opacity-100 scale-95" 
            : "border-transparent opacity-50 hover:opacity-100"
          }`}
        >
          <img src={img.url} className="w-full h-full object-cover" alt={`Küçük resim ${i}`} />
        </button>
      ))}
    </div>
  )}
            ...
            {/* TEKNİK ÖZELLİKLER (Dolu olanlar gözükür) */}
            <div className="flex flex-col gap-6">
               <h3 className="text-[1.25rem] font-light text-[#0A1628] tracking-tight border-b border-[#112769]/10 pb-4 uppercase tracking-[0.1em]">Teknik Özellikler</h3>
            ...
            {/* AÇIKLAMA */}
            <div className="flex flex-col gap-6">
              <h3 className="text-[1.25rem] font-light text-[#0A1628] tracking-tight border-b border-[#112769]/10 pb-4 uppercase tracking-[0.1em]">İlan Açıklaması</h3>
          ...
          <div className="lg:col-span-4 lg:sticky lg:top-32 flex flex-col gap-8">
            <div className="bg-[#112769] p-10 text-white flex flex-col gap-8 shadow-2xl shadow-[#112769]/10 relative overflow-hidden">
               ...
               <div className="relative z-10 flex flex-col items-center text-center gap-5">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <span className="text-[11px] font-medium tracking-[0.25em] text-[#C5A572] uppercase">Gayrimenkul Danışmanı</span>
                  </div>
               </div>

               <div className="relative z-10 flex flex-col gap-4">
                 <a href="tel:+905397445120" className="group relative w-full py-4 bg-white text-[#112769] text-[12px] font-bold tracking-widest uppercase flex items-center justify-center gap-3 overflow-hidden transition-all duration-500 hover:text-white">
                 ...
                 <a href="https://wa.me/+905397445120" target="_blank" rel="noreferrer" className="w-full py-4 border border-white/20 text-white text-[12px] font-bold tracking-widest uppercase flex items-center justify-center gap-3 hover:border-[#C5A572] hover:text-[#C5A572] transition-all duration-500">
                    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M12.012 2c-5.506 0-9.989 4.478-9.99 9.984a9.964 9.964 0 001.333 4.993L2 22l5.135-1.348a9.944 9.944 0 004.877 1.28h.005c5.507 0 9.99-4.478 9.99-9.985 0-2.667-1.037-5.176-2.925-7.062 -2.926-7.062a9.94 9.94 0 00-7.07-2.923z"/></svg>
                    WhatsApp İle Yaz
                 </a>
               </div>
            </div>
            ...
            <div className="bg-white p-8 border border-[#112769]/5 shadow-sm">
              ...
              <div className="flex items-center gap-3">
                <svg className="w-5 h-5 text-[#C5A572]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                <span className="text-[14px] text-[#4A5568] font-light leading-relaxed">{listing.district} / {listing.city}</span>
              </div>
            </div>
          </div>
