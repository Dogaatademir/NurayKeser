...
const HomePage: React.FC = () => {
  // Ana sayfa SEO yönetimi
  useEffect(() => {
    const title = "Nuray Keser | Gayrimenkul Danışmanı";
    const description =
      "Nuray Keser ile satılık ve kiralık gayrimenkul ilanlarını inceleyin. Profesyonel danışmanlık, seçkin portföyler ve hızlı iletişim için resmi web sitesini ziyaret edin.";
  ...
  // Sayfa içi anchor desteği (/, /#iletisim gibi)
  useEffect(() => {
    const supportedIds = new Set(["ilanlar", "servisler", "iletisim"]);

    const tryScrollToHash = (retries = 24) => {
      const rawHash = window.location.hash.replace(/^#/, "");
      if (!supportedIds.has(rawHash)) return;

      const el = document.getElementById(rawHash);
      if (el) {
        el.scrollIntoView({ behavior: "smooth", block: "start" });
      } else if (retries > 0) {
        window.setTimeout(() => tryScrollToHash(retries - 1), 50);
      }
    };

    const timer = window.setTimeout(() => tryScrollToHash(), 0);
    ...
    <div className="bg-[#FAFAFA]">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-20 items-center">
            <div className="lg:col-span-7 flex flex-col gap-10">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[11px] font-medium tracking-[0.2em] text-[#112769] uppercase">
      ...
      <section id="ilanlar" className={`py-24 md:py-32 ${revealClass}`}>
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          ...
          {loadingFeatured ? (
            <div className="py-32 text-center text-[#8B92A4] font-light text-sm tracking-wide">
              Portföy yükleniyor...
            </div>
          ) : featuredError ? (
            ...
            <div className="py-32 text-center text-[#8B92A4] font-light text-sm tracking-wide">
              Şu anda görüntülenecek ilan bulunmamaktadır.
            </div>
          ) : (
            <div className="relative">
      ...
      <section
        id="servisler"
        className={`py-24 md:py-40 bg-white ${revealClass}`}
      ...
      >
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          <div className="mb-20">
            ...
            <h2 className="text-[2.5rem] font-extralight text-[#0A1628] tracking-tight max-w-2xl">Size Özel Çözümler</h2>
          </div>
                ...
                return (
                  <button
                    ...
                    className={`h-[3px] transition-all duration-500 rounded-full ${
                      isActive ? "w-10 bg-[#C5A572]" : "w-4 bg-white/20 hover:bg-white/40"
                    }`}
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-20 lg:gap-24">
            <div className="lg:col-span-5 flex flex-col">
              ...
              <h2 className="text-[2.5rem] font-extralight text-[#0A1628] tracking-tight mb-6 leading-tight">Görüşelim</h2>
                ...
                {[
                  ...
                  <div key={i} className="flex flex-col gap-3 group">
                    ...
                    <a
                      ...
                      target={item.label === "OFİS" ? "_blank" : "_self"}
                      rel={item.label === "OFİS" ? "noreferrer" : undefined}
                    >
                      {item.val}
                    </a>
                  </div>
